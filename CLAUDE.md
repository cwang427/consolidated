...
- Clips ping-pong rather than restarting: `playBoomerang()` plays forward natively, then steps `currentTime` back a frame at a time (no browser supports a negative `playbackRate`). The `<video>` therefore carries **no** `loop` attribute.
