...
- **Map tab filters:** the category chip row filters *both* idea and memory pins. A memory has no category field — it inherits one from its linked idea (`ideaId`); spontaneous dates and ones whose idea was deleted fall into a `Spontaneous` bucket (`NOCAT`) so a filter never hides them silently. Chips list only categories with pins currently on the map.
